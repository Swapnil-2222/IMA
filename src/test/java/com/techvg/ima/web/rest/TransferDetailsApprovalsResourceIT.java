...
import com.techvg.ima.repository.TransferDetailsApprovalsRepository;
import com.techvg.ima.service.TransferDetailsApprovalsService;
import com.techvg.ima.service.criteria.TransferDetailsApprovalsCriteria;
import com.techvg.ima.service.dto.TransferDetailsApprovalsDTO;
import com.techvg.ima.service.mapper.TransferDetailsApprovalsMapper;
...
class TransferDetailsApprovalsResourceIT {
    ...
    @Mock
    private TransferDetailsApprovalsRepository transferDetailsApprovalsRepositoryMock;

    @Autowired
    private TransferDetailsApprovalsMapper transferDetailsApprovalsMapper;

    @Mock
    private TransferDetailsApprovalsService transferDetailsApprovalsServiceMock;
    ...
    void getAllTransferDetailsApprovalsWithEagerRelationshipsIsEnabled() throws Exception {
        when(transferDetailsApprovalsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restTransferDetailsApprovalsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(transferDetailsApprovalsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllTransferDetailsApprovalsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(transferDetailsApprovalsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restTransferDetailsApprovalsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(transferDetailsApprovalsRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void putExistingTransferDetailsApprovals() throws Exception {
        // Initialize the database
        transferDetailsApprovalsRepository.saveAndFlush(transferDetailsApprovals);
    ...
    void deleteTransferDetailsApprovals() throws Exception {
        ...
        restTransferDetailsApprovalsMockMvc
            .perform(delete(ENTITY_API_URL_ID, transferDetailsApprovals.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
