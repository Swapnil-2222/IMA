...
import com.techvg.ima.domain.enumeration.AccessLevel;
import com.techvg.ima.repository.UserAccessRepository;
import com.techvg.ima.service.UserAccessService;
import com.techvg.ima.service.criteria.UserAccessCriteria;
import com.techvg.ima.service.dto.UserAccessDTO;
import com.techvg.ima.service.mapper.UserAccessMapper;
...
class UserAccessResourceIT {
    ...
    @Mock
    private UserAccessRepository userAccessRepositoryMock;
    ...
    void getAllUserAccessesWithEagerRelationshipsIsEnabled() throws Exception {
        when(userAccessServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restUserAccessMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(userAccessServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllUserAccessesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(userAccessServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restUserAccessMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(userAccessRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void deleteUserAccess() throws Exception {
        ...
        restUserAccessMockMvc
            .perform(delete(ENTITY_API_URL_ID, userAccess.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
