...
import com.techvg.ima.repository.ConsumptionDetailsRepository;
import com.techvg.ima.service.ConsumptionDetailsService;
import com.techvg.ima.service.criteria.ConsumptionDetailsCriteria;
import com.techvg.ima.service.dto.ConsumptionDetailsDTO;
import com.techvg.ima.service.mapper.ConsumptionDetailsMapper;
...
class ConsumptionDetailsResourceIT {
    ...
    @Mock
    private ConsumptionDetailsRepository consumptionDetailsRepositoryMock;

    @Autowired
    private ConsumptionDetailsMapper consumptionDetailsMapper;

    @Mock
    private ConsumptionDetailsService consumptionDetailsServiceMock;
    ...
    void getAllConsumptionDetailsWithEagerRelationshipsIsEnabled() throws Exception {
        when(consumptionDetailsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restConsumptionDetailsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(consumptionDetailsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllConsumptionDetailsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(consumptionDetailsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restConsumptionDetailsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(consumptionDetailsRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void getAllConsumptionDetailsByLastModifiedContainsSomething() throws Exception {
        // Initialize the database
        consumptionDetailsRepository.saveAndFlush(consumptionDetails);

        // Get all the consumptionDetailsList where lastModified contains DEFAULT_LAST_MODIFIED
        defaultConsumptionDetailsShouldBeFound("lastModified.contains=" + DEFAULT_LAST_MODIFIED);

        // Get all the consumptionDetailsList where lastModified contains UPDATED_LAST_MODIFIED
        defaultConsumptionDetailsShouldNotBeFound("lastModified.contains=" + UPDATED_LAST_MODIFIED);
    }

    @Test
    @Transactional
    void getAllConsumptionDetailsByLastModifiedNotContainsSomething() throws Exception {
        // Initialize the database
        consumptionDetailsRepository.saveAndFlush(consumptionDetails);

        // Get all the consumptionDetailsList where lastModified does not contain DEFAULT_LAST_MODIFIED
        defaultConsumptionDetailsShouldNotBeFound("lastModified.doesNotContain=" + DEFAULT_LAST_MODIFIED);

        // Get all the consumptionDetailsList where lastModified does not contain UPDATED_LAST_MODIFIED
        defaultConsumptionDetailsShouldBeFound("lastModified.doesNotContain=" + UPDATED_LAST_MODIFIED);
    }
    ...
    void deleteConsumptionDetails() throws Exception {
        ...
        restConsumptionDetailsMockMvc
            .perform(delete(ENTITY_API_URL_ID, consumptionDetails.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
