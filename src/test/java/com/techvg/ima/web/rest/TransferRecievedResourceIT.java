...
import com.techvg.ima.repository.TransferRecievedRepository;
import com.techvg.ima.service.TransferRecievedService;
import com.techvg.ima.service.criteria.TransferRecievedCriteria;
import com.techvg.ima.service.dto.TransferRecievedDTO;
import com.techvg.ima.service.mapper.TransferRecievedMapper;
...
class TransferRecievedResourceIT {
    ...
    @Mock
    private TransferRecievedRepository transferRecievedRepositoryMock;

    @Autowired
    private TransferRecievedMapper transferRecievedMapper;

    @Mock
    private TransferRecievedService transferRecievedServiceMock;
    ...
    void getAllTransferRecievedsWithEagerRelationshipsIsEnabled() throws Exception {
        when(transferRecievedServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restTransferRecievedMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(transferRecievedServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllTransferRecievedsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(transferRecievedServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restTransferRecievedMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(transferRecievedRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void putExistingTransferRecieved() throws Exception {
        // Initialize the database
        transferRecievedRepository.saveAndFlush(transferRecieved);
    ...
    void deleteTransferRecieved() throws Exception {
        ...
        restTransferRecievedMockMvc
            .perform(delete(ENTITY_API_URL_ID, transferRecieved.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
